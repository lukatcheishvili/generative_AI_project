...
Python port of web/lib/framers.ts.
...
`resolve_framer_id`). The user can override the choice in the plan card.
"""
...
def get_framer(framer_id: Optional[str]) -> Optional[dict]:
    if not framer_id:
        return None
